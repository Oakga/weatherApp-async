...
export function* callGeoAPI(action) {
  ...
  try {
    const response = yield call(API.geoAPI, urlString);
    yield put(geoFetched(response, action.date));
  } catch (err) {
...
const API = {
  ...
  darkSkyAPI(url) {
    ...
    .then((response) => response.json())
    .then((data) => {
      const todayWeather = data.currently;
      const weeklyWeather = data.daily.data;
      return {
        todayWeather,
        weeklyWeather,
      };
