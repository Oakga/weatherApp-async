...
function homeReducer(state = initialState, action) {
  const previousSearchHistory = state.searchHistory;
  ...
  switch (action.type) {
    ...
    // for the first time search i.e if currentSearch is null or if the search already been done ,don't put the old search in history for the new search
      if (oldSearch.trim().length > 0 || !previousSearchHistory.includes(action.location)) {
        previousSearchHistory.push(oldSearch);
      }
      return state
